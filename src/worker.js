...
app.get("/api/cars",async c=>{if(!c.env.DB)return c.json({error:"D1 databáza DB nie je pripojená k Workeru."},503);try{const {results=[]}=await c.env.DB.prepare("SELECT cars.id,cars.title,cars.subtitle,cars.year,cars.km,cars.power,cars.range_km,cars.drive,cars.color,cars.price_eur,cars.status,cars.description,(SELECT url FROM photos p WHERE p.car_id=cars.id ORDER BY p.position LIMIT 1) AS cover_url FROM cars ORDER BY created_at DESC").all();return c.json({cars:results})}catch(e){return c.json({error:"Chyba D1: "+String(e?.message||e)},500)}});
...
if(parsed.color)await c.env.DB.prepare("UPDATE cars SET color=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(parsed.color,id).run();
